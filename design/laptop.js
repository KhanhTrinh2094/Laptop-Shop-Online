...
// Gui phan hoi
$(document).on('click','.gopy#dell',function(){
    $.ajax({
        url:'view/gopy.asp',
        type:'get',
...
$(document).on('click','.xoa#gopy',function(){
    if($('#tieude').val().trim()=="" || $('#nd').val().trim()==""){
	...
	var time = new Date().getTime();
    $.ajax({
        url:'view/gopy.asp?time='+new Date().getTime(),
        type:'get',
		data: {tieude:tieude, nd:nd, time:time, id: id},
        success:function(data){
...
// Thanh toan
$(document).on('click','#guidon',function(){
var checkmail=/^[A-Za-z][A-Za-z0-9]{4,20}@[A-Za-z]+\.[A-Za-z]{2,4}(.[A-Za-z]{2,4})*$/;
var checkdt=/^[0-9]{10,11}$/;
...
$(document).on('click','a#dell.tintuc',function(){
    ...
    $(document).on('change','input#soluong',function(){
        
        if(!checksl.test(soluong.value)){
            $('#angiohang').load('view/buy.php');
            return false;
        }
        $.ajax({
            url:'view/buy.php',
            type:'post',
            data:'actions=capnhat&mss='+$(this).attr('name')+'&soluongs='+$(this).val(),
            success:function(){
                $('#angiohang').load('view/buy.php');
            }
        })
...
$(document).on('click','input#check',function(){
    ...
    $.ajax({
        url:'view/khachthanhtoan.php',
        type:'post',
        data:'rname='+$('#hoten').val()+'&rgioitinh='+$('#gt').val()+'&remail='+$('#mail').val()+'&rdienthoai='+$('#dienthoai').val()+'&rdiachi='+$('#diachi').val(),
        success:function(ok){
            ...
            $('.anttk').load('view/khachthanhtoan.php').slideDown();
...
$(document).on('click','a.dell#delhd',function(){
    $.ajax({
        url:'view/lichsugd.php',
        type:'post',
        data:'mahd='+$(this).attr('mahd'),
        success:function(okk){
            $('#anls').hide();
            $('#anls').load('view/lichsugd.php');
            $('#anls').slideDown();
        }
    })
})

$(document).on('click','#guiph',function(){
        $('#anthongbao').slideUp();
        $('#anthongbao').load('view/phanhoi.php');
        $('#anthongbao').slideDown();
    })



$(document).on('click','.xoa#guiphanhoi',function(){
    $.ajax({
        url:'view/phanhoi.php',
        type:'post',
        data:'tieude='+$('#tieudeph').val()+'&noidung='+$('#noidungph').val(),
        success:function(){
            $('#angui').hide();
            $('#anthongbao').load('view/thongbao.php').slideDown();
        }

    })
})

$(document).on('click','.dell#phanhoi',function(){
    $.ajax({
        url:'view/showthongbao.php',
        type:'post',
        data:'matin='+$(this).attr('matin'),
        success:function(ok){
            $('#load').html(ok).fadeIn('slow');
            $('.demtb').load('view/demthongbao.php');
        }
    })
})
...
$(document).on('click','.close',function(){
    $('#load').fadeOut('slow');
    $('#anthongbao').hide();
    $('#anthongbao').load('view/thongbao.php').show();
...
$(document).on('click','a.dell#hdh',function(){
    $('#loadhome').html('<img src="design/loadhome.gif" />').show();
    $.ajax({
        url:'view/home.php',
        type:'post',
        data:'mahdh='+$(this).attr('mahdh'),
        success:function(data){
            $('.anhome').empty();
            $('.columnCenter').empty().append(data);
            $('#loadhome').fadeOut();
...
$(document).on('click','.dell#lichsu',function(){
    $('.anhome').empty();
    $('.columnCenter').hide().slideDown().empty().load('view/lichsugd.php');
})

$(document).on('click','.dell#thongbao',function(){
    $('.anhome').empty();
    $('.columnCenter').hide().slideDown().load('view/thongbao.php');
})
...
$(document).on('blur','input#hoten',function(){
    if($(this).val()==''){
        $('.hotenck').empty();
        $('<br/><span id="check" class="hotenck">Không được để trống!</span>').insertAfter(this);
    }
    else
        $('.hotenck').empty();
})
...
$(document).on('blur','#mail',function(){
    if($(this).val()==''){
        $('.mailck').empty();
        $('<br/><span class="mailck" id="check">Không được để trống!</span>').insertAfter(this);
    }
    else
        $('.mailck').empty();
})
