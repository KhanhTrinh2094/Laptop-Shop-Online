...
Dim DEL_BL
    Set DEL_BL = Server.CreateObject ("ADODB.Command")
    DEL_BL.ActiveConnection = MM_cn_STRING
    DEL_BL.CommandText = "DELETE FROM dbo.tbBinhLuan WHERE KHID = ?"
    DEL_BL.Parameters.Append DEL_BL.CreateParameter("param1", 5, 1, -1, Request.Form("MM_recordId")) ' adDouble
    DEL_BL.Execute
    DEL_BL.ActiveConnection.Close
End If
%>
<%
If (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
Dim DEL_PH
    ...
    DEL_PH.CommandText = "DELETE FROM dbo.tbPhanHoi WHERE KHID = ?"
...
If (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "DELETE FROM dbo.tbKhachHang WHERE ID = ?"
    ...
    MM_editRedirectUrl = "quanlythanhvien.asp?delete=ok"
    If (Request.QueryString <> "") Then
...
rsKhachhang_cmd.CommandText = "SELECT * FROM dbo.tbKhachHang WHERE ID = ?" 
rsKhachhang_cmd.Prepared = true
rsKhachhang_cmd.Parameters.Append rsKhachhang_cmd.CreateParameter("param1", 5, 1, -1, rsKhachhang__MMColParam) ' adDouble
...
rsDonHang_cmd.CommandText = "SELECT COUNT(*) as NUM FROM dbo.tbHoaDon WHERE KHID = ?" 
rsDonHang_cmd.Prepared = true
rsDonHang_cmd.Parameters.Append rsDonHang_cmd.CreateParameter("param1", 5, 1, -1, rsKhachhang__MMColParam) ' adDouble
...
<!-- InstanceBeginEditable name="EditRegion3" -->
                ...
                <div class="row-fluid">
                    ...
                    <div class="span12">
                      ...
                      <% if((rsDonhang.Fields.Item("NUM").Value) <= 0) then%>
                        <div class="span12" align="center"><h3>Bạn có chắc chắn muốn xóa thành viên : <%=(rsKhachhang.Fields.Item("TenDangNhap").Value)%></h3><br />
<form name="form1" method="POST" action="<%=MM_editAction%>">
    <input name="ID" type="hidden" id="ID" value="<%=(rsKhachhang.Fields.Item("ID").Value)%>">
    <input type="submit" name="submit" id="submit" value="Đồng ý">
  <input type="hidden" name="MM_delete" value="form1">
  <input type="hidden" name="MM_recordId" value="<%= rsKhachhang.Fields.Item("ID").Value %>">
</form>
<a href="quanlythanhvien.asp"><input type="button" value="Hủy bỏ"/></a>
</div>
<% else %>
<div class="span12" align="center"><h3>Thành viên này vẫn còn đơn hàng. Vui lòng xóa đơn hàng trước khi xóa thành viên</h3></div>
<% end if %>
</div>
</div>
