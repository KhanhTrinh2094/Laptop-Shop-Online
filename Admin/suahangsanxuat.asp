...
If (CStr(Request("MM_update")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "UPDATE dbo.tbHangSanXuat SET TenHang = ?, Web = ?, LoGo = ? WHERE ID = ?" 
    MM_editCmd.Prepared = true
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 202, 1, 50, Request.Form("txtHang")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param2", 201, 1, 70, Request.Form("txtWeb")) ' adLongVarChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param3", 201, 1, 70, Request.Form("txtLink")) ' adLongVarChar
    ...
    MM_editRedirectUrl = "quanlyhangsanxuat.asp?update=ok"
    If (Request.QueryString <> "") Then
...
function kt()
{
	var validTen = /^[a-zA-Z0-9-.\/_ :]{0,50}$/;
	var txtHang = document.form1.txtHang.value.trim();
	var txtWeb = document.form1.txtWeb.value.trim();
	...
	ext=ext.toLowerCase();
	if(validTen.test(txtHang) == false)
	{
		alert("Tên hãng sản xuất chỉ chứa chữ cái, số và tối đa 50 kí tự. Vui lòng nhập lại");
		document.form1.txtHang.focus();
		return false;
	}
	else if(txtHang.length == 0)
	{
		alert("Vui lòng nhập tên hãng sản xuất");
		document.form1.txtHang.focus();
		return false;
	}
	else if(txtWeb.length == 0)
	{
		alert("Vui lòng Nhập địa chỉ web của nhà sản xuất");
		document.form1.txtWeb.focus();
		return false;
	}
	else if(txtLink.length == 0)
	{
		alert("Vui lòng chọn Logo cho hãng sản xuất này");
                ...
                <div class="row-fluid">
                    ...
                    <div class="span12">
                        <div class="head clearfix">
                            ...
                            <h1>Sửa hãng sản xuất : <%=(rsHSX.Fields.Item("TenHang").Value)%></h1>
                        </div>
                        <div class="block-fluid">
<form action="<%=MM_editAction%>" method="POST" name="form1" onsubmit="return kt()">                        
                            <div class="row-form clearfix">
                                <div class="span3">Tên hãng:</div>
                                <div class="span9"><input name="txtHang" type="text" required id="txtHang" value="<%=(rsHSX.Fields.Item("TenHang").Value)%>"/></div>
                            </div>
                            <div class="row-form clearfix">
                                <div class="span3">Trang Web:</div>
                              <div class="span3"><input name="txtWeb" type="text" required id="txtWeb" value="<%=(rsHSX.Fields.Item("Web").Value)%>"/></div>
                            </div>
                            <div class="row-form clearfix">
                                <div class="span3">Logo:</div>
                                <div class="span4">
                                  <img src="../thumb/HSX/<%=(rsHSX.Fields.Item("LoGo").Value)%>" alt="thumb" />
                                  <input type="file" name="txtImage" id="txtImage" onchange="cut_string();">
                                  <input type="hidden" name="txtLink" id="txtLink" value="<%=(rsHSX.Fields.Item("LoGo").Value)%>">
                                </div>
                            </div>
            ...
            <input type="hidden" name="MM_recordId" value="<%= rsHSX.Fields.Item("ID").Value %>">
</form>
    </div>
    </div>
    </div>
