...
If (Request.QueryString("action") = "lock") Then
    ...
    MM_editCmd.CommandText = "UPDATE dbo.tbKhachHang SET TrangThai = 0 WHERE ID = ?" 
    ...
    MM_editRedirectUrl = "quanlythanhvien.asp?update=ok"
...
If (Request.QueryString("action") = "unlock") Then
    Set MM_editCmd = Server.CreateObject ("ADODB.Command")
    MM_editCmd.ActiveConnection = MM_cn_STRING
    MM_editCmd.CommandText = "UPDATE dbo.tbKhachHang SET TrangThai = 1 WHERE ID = ?" 
    MM_editCmd.Prepared = true
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 202, 1, 50, Request.QueryString("ID")) ' adInteger
    MM_editCmd.Execute
    MM_editCmd.ActiveConnection.Close
    ...
    ' append the query string to the redirect URL
    MM_editRedirectUrl = "quanlythanhvien.asp?unlock=ok"
    If (Request.QueryString <> "") Then
      If (InStr(1, MM_editRedirectUrl, "?", vbTextCompare) = 0) Then
        MM_editRedirectUrl = MM_editRedirectUrl & "?" & Request.QueryString
      Else
