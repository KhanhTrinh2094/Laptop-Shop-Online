...
rsHSX_cmd.CommandText = "SELECT ID, TenHang FROM dbo.tbHangSanXuat WHERE ID = ?" 
...
rsSanpham_cmd.CommandText = "SELECT * FROM dbo.tbSanPham WHERE HSXID = ?" 
...
<!-- InstanceBeginEditable name="doctitle" -->
    ...
    <title>Phân loại hãng sản xuất</title>
<!-- InstanceEndEditable -->
...
<div class="page-header">
                   <h1><%=(rsHSX.Fields.Item("TenHang").Value)%> <small>Product</small></h1>
                </div>
                <div class="row-fluid">
                    ...
                    <div class="span12">                    
                        <div class="head clearfix">
                            ...
                            <h1><%=(rsHSX.Fields.Item("TenHang").Value)%> Product</h1>
                            <ul class="buttons">
                                <li><a href="themsanpham.asp" class="isw-plus"></a></li>
  ...
  <% While ((Repeat1__numRows <> 0) AND (NOT rsSanpham.EOF)) %>
                                    <tr>
                                    	<td><%=(rsSanpham.Fields.Item("ID").Value)%></td>
                                        <td><a href="thongtinsanpham.asp?id=<%=(rsSanpham.Fields.Item("ID").Value)%>"><%=(rsSanpham.Fields.Item("TenSanPham").Value)%></a></td>
                                        <td><%=(rsSanpham.Fields.Item("Chip").Value)%></td>
                                        <td><%=(rsSanpham.Fields.Item("HDD").Value)%></td>
                                        <td><%=(rsSanpham.Fields.Item("Ram").Value)%></td>
                                        <td><%=(rsSanpham.Fields.Item("GiaSanPham").Value)%></td>
                                                                                <td><% if(rsSanpham.Fields.Item("TrangThai").Value) = 1 Then
...
%></td>
                                        <td><div class="btn-group" align="center">                                        
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="suasanpham.asp?id=<%=(rsSanpham.Fields.Item("ID").Value)%>">Sửa</a></li>
                                                
                                                <li class="divider"></li>
                                                
                                                <li><a href="xoasanpham.asp?id=<%=(rsSanpham.Fields.Item("ID").Value)%>">Xóa</a></li>
                                            </ul>
                                        </div></td>
                                    </tr>
    <% 
