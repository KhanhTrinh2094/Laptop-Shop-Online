...
<%
Dim rsPhanhoi__MMColParam
rsPhanhoi__MMColParam = "1"
If (Request.QueryString("ID") <> "") Then 
  rsPhanhoi__MMColParam = Request.QueryString("ID")
End If
%>
<%
Dim rsPhanhoi
Dim rsPhanhoi_cmd
Dim rsPhanhoi_numRows

Set rsPhanhoi_cmd = Server.CreateObject ("ADODB.Command")
rsPhanhoi_cmd.ActiveConnection = MM_cn_STRING
rsPhanhoi_cmd.CommandText = "SELECT a.*, b.TenDangNhap FROM tbPhanHoi a INNER JOIN tbKhachHang b ON a.KHID = b.ID WHERE a.ID = ?" 
rsPhanhoi_cmd.Prepared = true
rsPhanhoi_cmd.Parameters.Append rsPhanhoi_cmd.CreateParameter("param1", 5, 1, -1, rsPhanhoi__MMColParam) ' adDouble

Set rsPhanhoi = rsPhanhoi_cmd.Execute
rsPhanhoi_numRows = 0
%>
<%
...
Repeat1__numRows = -1
Repeat1__index = 0
rsPhanhoi_numRows = rsPhanhoi_numRows + Repeat1__numRows
%>
<!DOCTYPE html>
                    ...
                    <div class="span12">
                        ...
                        <div class="block-fluid">                        
                            <div class="row-form clearfix">
                                ...
                                <div class="span9"><%=(rsPhanhoi.Fields.Item("TieuDe").Value)%></div>
                            </div>
                            <div class="row-form clearfix">
                                <div class="span3">Thành viên:</div>
                                <div class="span9"><a href="thongtinthanhvien.asp?id=<%=(rsPhanhoi.Fields.Item("KHID").Value)%>"><%=(rsPhanhoi.Fields.Item("TenDangNhap").Value)%></a></div>
                            </div>
                            <div class="row-form clearfix">
                                <div class="span3">Thời gian:</div>
                                <div class="span9"><%=(rsPhanhoi.Fields.Item("ThoiGian").Value)%></div>
                            </div>
                            <div class="row-form clearfix">
                                <div class="span3">Nội dung:</div>
                                <div class="span9"><%=(rsPhanhoi.Fields.Item("NoiDung").Value)%></div>
                            </div>
                            <div class="row-form clearfix">
                                <div class="span3">Thao tác:</div>
                                <div class="span9"><a href="xoaphanhoi.asp?id=<%=(rsPhanhoi.Fields.Item("ID").Value)%>"><button class="btn" type="button">Xóa</button></a></div>
                            </div>
                       </div>
                   </div>
               </div>
