...
rsHoaDon_cmd.CommandText = "SELECT * FROM dbo.tbHoaDon" 
...
Repeat1__numRows = -1
Repeat1__index = 0
rsHoaDon_numRows = rsHoaDon_numRows + Repeat1__numRows
%>
<%
Dim rsKhachhang
Dim rsKhachhang_cmd
...
rsKhachhang_cmd.CommandText = "SELECT ID, TenDangNhap FROM dbo.tbKhachHang WHERE ID = ?" 
                    ...
                    <div class="span12">                    
                        ...
                        <div class="block-fluid table-sorting clearfix">
                            <table cellpadding="0" cellspacing="0" width="100%" class="table" id="example">
                                ...
                                <tbody>
  <% While ((Repeat1__numRows <> 0) AND (NOT rsHoaDon.EOF)) %>
    <tr>
      <td><a href="thongtindonhang.asp?id=<%=(rsHoaDon.Fields.Item("ID").Value)%>"><%=(rsHoaDon.Fields.Item("ID").Value)%></a></td>
      <td><%=(rsHoaDon.Fields.Item("TenNguoiNhan").Value)%></td>
      <td><a href="thongtinthanhvien.asp?id=<%=(rsKhachhang.Fields.Item("ID").Value)%>"><%=(rsKhachhang.Fields.Item("TenDangNhap").Value)%></a></td>
      <td><%=(rsHoaDon.Fields.Item("DiaChi").Value)%></td>
      <td><%=(rsHoaDon.Fields.Item("NgayGiaoHang").Value)%></td>
      <td><% if((rsHoaDon.Fields.Item("TrangThai").Value) = 1) then %>Đã xử lý<%else%>Chưa xử lý<% end if %></td>
      <td><div class="btn-group" align="center">                                        
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="xoadonhang.asp?id=<%=(rsHoaDon.Fields.Item("ID").Value)%>">Xóa</a></li>
                                            </ul>
                                        </div></td>
    </tr>
    <% 
