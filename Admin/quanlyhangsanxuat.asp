	...
	<script type="text/javascript" charset="utf-8">
			$(document).ready(function() {
                ...
                <div class="alert alert-success">                
                    <h4>Thành công!</h4>
                    Sửa hãng sản xuất thành công.
                </div> 
                <% end if %>
                <% if((Request.QueryString("delete")) = "ok") then %>
                <div class="alert alert-success">                
                    <h4>Thành công!</h4>
                    Xóa hãng sản xuất thành công.
                </div>
                <% end if %>
                <% if((Request.QueryString("inser")) = "ok") then %>
                <div class="alert alert-success">                
                    <h4>Thành công!</h4>
                ...
                <div class="row-fluid">
                    ...
                    <div class="span12">                    
                        <div class="head clearfix">
                            ...
                            <h1>Quản lý hãng sản xuất</h1>
                            <ul class="buttons">
                                <li><a href="themhangsanxuat.asp" class="isw-plus"></a></li>
...
rsCount_cmd.ActiveConnection = MM_cn_STRING
rsCount_cmd.CommandText = "SELECT COUNT(*) as num FROM dbo.tbSanPham WHERE HSXID = ?" 
rsCount_cmd.Parameters.Append rsCount_cmd.CreateParameter("param1", 5, 1, -1, (rsHSX.Fields.Item("ID").Value)) ' adDouble
...
%>
                                    <tr>
                                   	  <td><%=(rsHSX.Fields.Item("ID").Value)%></td>
                                        <td><h5><a href="thongtinhangsanxuat.asp?id=<%=(rsHSX.Fields.Item("ID").Value)%>"><%=(rsHSX.Fields.Item("TenHang").Value)%></a></h5></td>
                                        <td><center>
                                        <% if((rsCount.Fields.Item("num").Value) = 0) then %>
                                        <%=(rsCount.Fields.Item("num").Value)%>
                                        <% else %>
                                        <a href="soluongsanpham.asp?id=<%=(rsHSX.Fields.Item("ID").Value)%>"><%=(rsCount.Fields.Item("num").Value)%></a>
                                        <% end if %>
                                        </center></td>
                                      <td><a href="<%=(rsHSX.Fields.Item("Web").Value)%>"><%=(rsHSX.Fields.Item("Web").Value)%></a></td>
                                        
                                      <td width="35%" class="ui-button-text"><center><img src="../thumb/HSX/<%=(rsHSX.Fields.Item("Logo").Value)%>" alt="logo" height="30px" width="120px"/></center></td>
                                        <td><div class="btn-group" align="center">                                        
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="suahangsanxuat.asp?id=<%=(rsHSX.Fields.Item("ID").Value)%>">Sửa</a></li>
                                                
                                                <li class="divider"></li>
                                                
                                                <li><a href="xoahangsanxuat.asp?id=<%=(rsHSX.Fields.Item("ID").Value)%>">Xóa</a></li>
                                            </ul>
                                        </div></td>
                                    </tr>
                      <% 
