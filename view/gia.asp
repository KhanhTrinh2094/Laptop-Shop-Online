...
rsSanpham_cmd.CommandText = "SELECT * FROM dbo.tbSanPham WHERE GiaSanPham >= ? AND GiaSanPham < ?" 
rsSanpham_cmd.Prepared = true
rsSanpham_cmd.Parameters.Append rsSanpham_cmd.CreateParameter("param1", 200, 1, 255, Giathanh1) ' adVarChar
rsSanpham_cmd.Parameters.Append rsSanpham_cmd.CreateParameter("param2", 200, 1, 255, Giathanh2) ' adVarChar
...
<% if((MM_rsCount) = 0) then %>
   <div class="no-item">
Không có sản phẩm có giá thành phù hợp
</div>
<% end if %>
...
<table border="0">
  <tr>
    <td><% If MM_offset <> 0 Then %>
        <a class="pagegia" id="dell" page="<%=MM_moveFirst%>" gia="<%=Request.QueryString("ID")%>">First</a>
        <% End If ' end MM_offset <> 0 %></td>
    <td><% If MM_offset <> 0 Then %>
        <a class="pagegia" id="dell" page="<%=MM_movePrev%>" gia="<%=Request.QueryString("ID")%>">Previous</a>
        <% End If ' end MM_offset <> 0 %></td>
    <td><% If Not MM_atTotal Then %>
        <a class="pagegia" id="dell" page="<%=MM_moveNext%>" gia="<%=Request.QueryString("ID")%>">Next</a>
        <% End If ' end Not MM_atTotal %></td>
    <td><% If Not MM_atTotal Then %>
        <a class="pagegia" id="dell" page="<%=MM_moveLast%>" gia="<%=Request.QueryString("ID")%>">Last</a>
        <% End If ' end Not MM_atTotal %></td>
  </tr>
</table>
</div>
