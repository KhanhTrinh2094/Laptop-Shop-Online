...
If (Request.QueryString("ID") = "") Then
	response.Write("Tên đăng nhập không được để trống")
end if
%>
<%
Dim rsKiemtraid__MMColParam
rsKiemtraid__MMColParam = "1"
If (Request.QueryString("ID") <> "") Then
  rsKiemtraid__MMColParam = Request.QueryString("ID")
End If
%>
<%
Dim rsKiemtraid
...
rsKiemtraid_cmd.CommandText = "SELECT * FROM dbo.tbKhachHang WHERE TenDangNhap = ?" 
rsKiemtraid_cmd.Prepared = true
rsKiemtraid_cmd.Parameters.Append rsKiemtraid_cmd.CreateParameter("param1", 200, 1, 255, rsKiemtraid__MMColParam) ' adDouble
...
<%
Dim Nick
Nick = Request.QueryString("ID")
if((len(Nick)) < 6 or (len(Nick)) > 12) then
response.Write("Độ dài tên đăng nhập phải lớn hơn 6, nhỏ hơn 12")
else
if ((MM_rsCount) > 0) then
response.Write("Tên đăng nhập này đã có người sử dụng !")
else
response.Write("Bạn có thể sử dụng tên đăng nhập này !")
end if
end if
%>
