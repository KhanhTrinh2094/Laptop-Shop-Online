...
rsSanpham_cmd.CommandText = "SELECT * FROM dbo.tbSanPham WHERE Ram >= ? AND Ram < ?" 
rsSanpham_cmd.Prepared = true
rsSanpham_cmd.Parameters.Append rsSanpham_cmd.CreateParameter("param1", 200, 1, 255, Ram1) ' adVarChar
rsSanpham_cmd.Parameters.Append rsSanpham_cmd.CreateParameter("param2", 200, 1, 255, Ram2) ' adVarChar
...
<% if((MM_rsCount) = 0) then %>
   <div class="no-item">
Không có sản phẩm có dung lượng Ram phù hợp
</div>
<% end if %>
...
<table border="0">
  <tr>
    <td><% If MM_offset <> 0 Then %>
        <a class="pageram" id="dell" page="<%=MM_moveFirst%>" ram="<%=Request.QueryString("ID")%>">First</a>
        <% End If ' end MM_offset <> 0 %></td>
    <td><% If MM_offset <> 0 Then %>
        <a class="pageram" id="dell" page="<%=MM_movePrev%>" ram="<%=Request.QueryString("ID")%>">Previous</a>
        <% End If ' end MM_offset <> 0 %></td>
    <td><% If Not MM_atTotal Then %>
        <a class="pageram" id="dell" page="<%=MM_moveNext%>" ram="<%=Request.QueryString("ID")%>">Next</a>
        <% End If ' end Not MM_atTotal %></td>
    <td><% If Not MM_atTotal Then %>
        <a class="pageram" id="dell" page="<%=MM_moveLast%>" ram="<%=Request.QueryString("ID")%>">Last</a>
        <% End If ' end Not MM_atTotal %></td>
  </tr>
</table>
</div>
